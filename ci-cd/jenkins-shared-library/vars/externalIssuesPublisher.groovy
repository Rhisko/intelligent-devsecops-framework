...
def call(Map args = [:]) {
    ...
    // 1. Validate arguments
    // --------------------------------------------------
    String tool      = args.tool ?: error("tool is required")
    String inputFile = args.input ?: error("input (<tool>.json) is required")
    ...
    // --------------------------------------------------
    // 2. Load RAW JSON (NO ASSUMPTION)
    // --------------------------------------------------
    def raw = readJSON file: inputFile

    // --------------------------------------------------
    // 3. Dispatch RAW payload to mapper
    // --------------------------------------------------
    Map sonarPayload
    ...
    // --------------------------------------------------
    if (!(sonarPayload?.rules instanceof List) ||
        !(sonarPayload?.issues instanceof List)) {
        error "[ExternalIssuesPublisher] Invalid Sonar payload from ${tool}"
    }

    echo "[ExternalIssuesPublisher] Rules : ${sonarPayload.rules.size()}"
    echo "[ExternalIssuesPublisher] Issues: ${sonarPayload.issues.size()}"

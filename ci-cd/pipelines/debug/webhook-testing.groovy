pipeline {
    ...
    triggers {
        GenericTrigger(
            genericVariables: [
                ...
                [key:'TAGS',value:'$.ref',expressionType:'JSONPath'],
            ...
            regexpFilterText: '',
            token:'GITHUB-WEBHOOK-TOKEN-DEVSECOPS-DEBUG',
    ...
    stages {
        stage("Setup Properties for Parameters") {
            steps {
                script {
                    properties([
                        parameters([
                            string(defaultValue: '', name: 'TAGS', description: 'Git reference tag', trim: true),
        ...
        stage("Checkout from Repository") {
            steps {
                script {
                    println "TAGS: ${TAGS}"
                    println "Checking out  repository from github"
                    // retry (1) {
                    //     sleep 2
                    //     sh 'git config --global http.sslVerify false'
                    //     git branch: 'master', credentialsId: 'credential-bitbucket', url: 'https://bitbucket.danamon.co.id/scm/infp/infra-provisioning.git'
                    // }
                }
            }
        }
    ...
    post {
        success {
            echo "[PIPELINE] SUCCESS for issue ${REPOSITORY_NAME} (${COMMIT_HASH_AFTER})"
        }
        failure {
            echo "[PIPELINE] FAILED for issue ${REPOSITORY_NAME} (${COMMIT_HASH_AFTER})"
        }
        always {
            script {
                def status = currentBuild.currentResult ?: 'UNKNOWN'
                currentBuild.displayName = "${status} - ${REPOSITORY_NAME}-${BASE_REF}-${COMMIT_HASH_AFTER}-${BUILD_NUMBER}"

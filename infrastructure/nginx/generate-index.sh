...
OUTPUT="/usr/share/nginx/html/index.html"
REPORT_ROOT="/usr/share/nginx/html"
...
<body>
  ...
  <main class="page">
    ...
    <div class="section-head">
      <h2 class="section-title">Available Findings Reports</h2>
      <div class="section-badge">Latest modified reports first</div>
    </div>
...
# Portable sort by modified time.
# This avoids "find -printf" because many nginx:alpine images use BusyBox find.
...
EOF
  ...
  TMP_SORT_FILE="/tmp/report-dirs-sort.txt"
  : > "$TMP_SORT_FILE"
  ...
  for dir in "$REPORT_ROOT"/*; do
    ...
    modified_epoch="$(stat -c '%Y' "$dir" 2>/dev/null || stat -f '%m' "$dir" 2>/dev/null || echo 0)"

    printf '%s|%s\n' "$modified_epoch" "$dir" >> "$TMP_SORT_FILE"
  done

  sort -t'|' -k1,1nr "$TMP_SORT_FILE" | while IFS='|' read -r modified_epoch dir; do
    [ -d "$dir" ] || continue

    name="$(basename "$dir")"

    modified_date="$(date -d "@$modified_epoch" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || date -r "$modified_epoch" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || echo 'Unknown')"
    ...
    cat >> "$OUTPUT" <<EOF
      <a class="card" href="/$name/">
        ...
        <div class="card-footer">
          <span class="tag">Security Findings</span>
          <span class="modified">Modified: $modified_date</span>
          <span class="action">Open Report</span>
        </div>
      </a>
EOF
